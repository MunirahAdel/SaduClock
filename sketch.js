...
function draw() {
  ...
  // vertical waves at specific hour positions
  drawVerticalWaves();
...
function drawVerticalWaves() {
  ...
  let waveAmplitude = 1; // wave height
  let waveFrequency = 0.5; // oscillation speed
  let waveSpeed = 0.05; //animation speed
  // Compute x positions
  let x1 = lerp(
    -half_w + margin,
    half_w - margin,
    hourPositions[0] / (parts[0] - 1)
  );
  let x2 = lerp(
    -half_w + margin,
    half_w - margin,
    hourPositions[1] / (parts[0] - 1)
  );

  // semi-transparent fill between these lines
  noStroke();
  ...
  for (let i = 0; i < hourPositions.length; i++) {
    ...
    for (let y = 0; y < height; y += 10) {
      let waveOffset =
        sin(y * waveFrequency + frameCount * waveSpeed) * waveAmplitude;
  ...
  for (let i = 0; i < timeParts.length; i++) {
    ...
    for (let j = 0; j < params.density; j++) {
      ...
      for (let yOffset = -w / 3; yOffset <= w / 3; yOffset += 100) {
        ...
        let wave =
          sin(
            yOffset * params.squiggleIntensity + x * 0.05 + frameCount * 0.2
          ) * 5;
...
function drawHorizontalSeparators() {
  for (let i = 0; i < timeParts.length - 1; i++) {
    ...
    for (let xOffset = -half_w; xOffset <= half_w; xOffset += 10) {
      ...
      if (i === 1) amplitude = 2;
      let curve =
        sin(xOffset * frequency + i * 0.2 + frameCount * frequency) * amplitude;
...
function drawWaves(
  ...
  let speedFactor = 4; // Increase for faster movement
  ...
  for (let i = 0; i < numLines; i++) {
    ...
    for (let y = 0; y < height; y++) {
      let x = map(
        noise(i + (y + offsetY) / 150, frameCount / (300 / speedFactor)),
        0,
        3,
        -half_w / 12,
        half_w / 3
